...
# On failure, writes a human-readable reason to stderr (for the import UI).
# Prefers synOCR's python3 venv (has the yaml module), then system python3, then yq_bin.
rules_yaml_to_json() {
    local _f="$1" _content _rc _py _errf _jq_out _jq_rc _jq_errf _last_err="" _wrap
    _wrap='if has("rules") and (.rules|type=="object") then {rules:.rules,groups:(.groups//{})} else {rules:.,groups:{}} end'
    [ -f "${_f}" ] || { printf '%s\n' "file not found: ${_f}" >&2; return 1; }
    _py=""
...
try:
    ...
    if mark is not None and "line" not in msg.lower():
        msg = "line %d, column %d: %s" % (mark.line + 1, mark.column + 1, msg)
    if "\\t" in msg or "\t" in msg:
        msg += "\nHint: YAML indentation must use spaces, not tab characters."
    sys.stderr.write(msg if msg.endswith("\n") else msg + "\n")
    sys.exit(1)' < "${_f}" 2>"${_errf}")
        _rc=$?
        if [ ${_rc} -eq 0 ] && [ -n "${_content}" ]; then
            _jq_errf=$(mktemp /tmp/synocr_ymlerr.XXXXXX)
            _jq_out=$(printf '%s' "${_content}" | jq -c "${_wrap}" 2>"${_jq_errf}")
            _jq_rc=$?
            ...
            _last_err=$(tr -d '\r' < "${_jq_errf}" 2>/dev/null)
            [ -z "${_last_err}" ] && _last_err="JSON wrap failed after YAML parse (jq exit ${_jq_rc})"
            rm -f "${_jq_errf}"
        else
            _last_err=$(tr -d '\r' < "${_errf}" 2>/dev/null)
            [ -z "${_last_err}" ] && _last_err="YAML parse failed"
        fi
        rm -f "${_errf}"
    fi
    if command -v yq_bin >/dev/null 2>&1; then
        _errf=$(mktemp /tmp/synocr_ymlerr.XXXXXX)
        _content=$(yq_bin read "${_f}" -jP 2>"${_errf}")
        _rc=$?
        if [ ${_rc} -eq 0 ] && [ -n "${_content}" ]; then
            _jq_errf=$(mktemp /tmp/synocr_ymlerr.XXXXXX)
            _jq_out=$(printf '%s' "${_content}" | jq -c "${_wrap}" 2>"${_jq_errf}")
            ...
            _last_err=$(tr -d '\r' < "${_jq_errf}" 2>/dev/null)
            [ -z "${_last_err}" ] && _last_err="JSON wrap failed after yq convert (jq exit ${_jq_rc})"
            rm -f "${_errf}" "${_jq_errf}"
            printf '%s\n' "${_last_err}" >&2
            return ${_jq_rc:-1}
        fi
        # Prefer the earlier Python/YAML detail when yq also fails.
        if [ -z "${_last_err}" ]; then
            _last_err=$(tr -d '\r' < "${_errf}" 2>/dev/null)
            [ -z "${_last_err}" ] && _last_err="yq could not read YAML file"
        fi
        rm -f "${_errf}"
        printf '%s\n' "${_last_err}" >&2
        return 1
...
rules_import_run() {
    local _id="$1" _path="${import_path:-}" _rj _blob _errs _rules _groups _count _yrc _vrc _errf
    if [ -z "${_path}" ]; then
    ...
    _errf=$(mktemp /tmp/synocr_ymlerr.XXXXXX)
    _blob=$(rules_yaml_to_json "${_path}" 2>"${_errf}")
    _yrc=$?
    _errs=$(tr -d '\r' < "${_errf}" 2>/dev/null)
    rm -f "${_errf}"
    if [ ${_yrc} -ne 0 ] || [ -z "${_blob}" ]; then
        ...
        echo '<div class="card card-body mb-3"><span class="text-danger">'"${lang_rules_import_error}"'</span>'
        if [ -n "${_errs}" ]; then
            echo '<pre class="small mt-2">'"$(rules_html_escape "${_errs}")"'</pre>'
        else
            echo '<p class="mb-0 mt-2">'"${lang_rules_import_invalid}"'</p>'
        fi
        echo '</div>'
        echo '<a href="index.cgi?page=rules-edit-'"${_id}"'&yaml=1" class="btn btn-secondary btn-sm">'"${lang_rules_back}"'</a>'
        return
...
rules_edit_view() {
    ...
    _lang_json=$(jq -c -n \
        ...
        --arg help_postscript_warn "${lang_rules_help_postscript_warn}" \
        --arg postscript_warn_short "${lang_rules_postscript_warn_short}" \
